...
void main() {		
	...
	int oldValue, newValue = 2, character = 3;
	int rowMove = 2, columnMove = 0, score = 0, moveNumber = 0;


	printf("\n\n\t\tWelcome !!\n\n\n\t\tThe purpose of the game is reaching to the end by going through less x\n");
	printf("\n\n\t\tW A S D for movement\n");
	printf("\n\n\t\tPress any key\t\t(2 Player)\n\n");
	time_t t;
	srand(time(NULL));
	...
	while (selected != 51) {
		...
		while (columnMove <= 14 && columnMove >= 0) {				//Continues as long as the character does not finish the map
			...
			if (wasdCoordinateChange == 100 || wasdCoordinateChange == 68) {				     //d
				...
				map[rowMove][columnMove] = character;			//yeni yerine characteri koyar
			}


			else if (wasdCoordinateChange == 97 || wasdCoordinateChange == 65) {				//a
			...
			else if (wasdCoordinateChange == 115 || wasdCoordinateChange == 83) {				//s
				map[rowMove][columnMove] = oldValue;
			...
			else if (wasdCoordinateChange == 119 || wasdCoordinateChange == 87) {				//w
				map[rowMove][columnMove] = oldValue;
